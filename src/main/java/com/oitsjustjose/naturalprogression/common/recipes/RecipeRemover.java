...
@Mod.EventBusSubscriber(modid = Constants.MODID) public class RecipeRemover
{
	private static final Field RECIPES = ObfuscationReflectionHelper
			.findField(RecipeManager.class, "field_199522_d" /* recipes */);

	@SubscribeEvent @SuppressWarnings("deprecation") public static void onServerStart(final FMLServerStartedEvent event)
	{
	...
	private static int removeRecipes(final RecipeManager recipeManager, final Predicate<IRecipe> predicate)
	{
		...
		try
		{
			@SuppressWarnings("unchecked") final Map<IRecipeType<?>, Map<ResourceLocation, IRecipe<?>>> recipesMap = (Map<IRecipeType<?>, Map<ResourceLocation, IRecipe<?>>>) RECIPES
					.get(recipeManager);
